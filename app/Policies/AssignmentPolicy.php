...
{
    function access(Teacher $teacher,Assignment $assignment)
    {
        return
            $teacher->courses()->with('assignments')->get()
                ->pluck('assignments')->flatten()->where('id', $assignment->id)->isNotEmpty()
            ? Response::allow()
            : Response::deny('You do not have permission to access on this assignment');
    }
    /**
